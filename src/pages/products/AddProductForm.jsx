...
const AddProductForm = () => {
	...
	const { data: aAttributeTypes, fetch: fetchAllAttributeTypes } =
		useAttributeTypes();
				...
				try {
					...
					if (response.data.STATUS === STATUS.SUCCESS.toUpperCase()) {
						...
						setFormData({
							...
							MRP: product.MRP,
							ProductDiscount:
								typeof product.productDiscount === "string"
										? parseFloat(product.productDiscount.replace("%", ""))
		...
		if (fetchedProduct && aAttributes.length > 0) {
			const fetchedVariants = fetchedProduct.variants.map((variant) => {
				...
				return {
					...
					images: variant.images.map((image, idx) => ({
						id: `${image}-${idx}`,
						file: null,
						preview: image,
					})),
	...
	const validateForm = () => {
		...
		if (!oFormData.StoreID)
			addError(
				"StoreID",
				t("PRODUCT_CREATION.STORE_ID_REQUIRED") || "Store is required"
			);
					...
					<DndContext
						...
						onDragEnd={({ active, over }) => {
							...
							setVariants((prevVariants) => {
								...
								newVariants[variantIndex].images = arrayMove(images, oldIndex, newIndex);
								return newVariants;
							});
						}}
					>
								...
								<div className="space-y-6">
									<div>
										<TextInputWithIcon
											...
											placeholder={t(
												"PRODUCT_CREATION.PRODUCT_NAME_PLACEHOLDER"
											)}
											Icon={Tag}
									...
									<div>
										<SelectWithIcon
											...
											onInputChange={(value) =>
												fetchAllAttributeTypes({ searchText: value })
											}
										/>
									</div>
									<div>
								...
								<div className="space-y-6">
									<div>
										<TextInputWithIcon
											...
											value={oFormData.MRP}
											onChange={handleInputChange}
											placeholder={t("PRODUCT_CREATION.MRP_PLACEHOLDER")}
											Icon={() => (
												<span
									...
									<div>
										<SelectWithIcon
											...
											options={[
												...
												{
													value: "Unisex",
													label: t("PRODUCT_CREATION.UNISEX"),
												},
							...
							<div className="grid grid-cols-1 md:grid-cols-2 gap-8">
								...
								<div>
									<SelectWithIcon
										...
										onInputChange={(value) =>
											fetchAllStores({ searchText: value })
										}
									/>
								</div>
							</div>
											...
											<div className="space-y-6">
												<div>
													<SelectWithIcon
														...
														onInputChange={(value) =>
															fetchAllColors({ searchText: value })
														}
